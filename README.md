...
当前版本：**v6**（`__WHALE_ROD_CURSOR_VER__ = '2026-09-15-v6-poses'`）
...
## 五、气泡：触发看加速度，字号看力度（v3 触发 / v4 字号）

写在 `mousemove` 事件监听里，就是需求说的那种做法：

```
sp  = 本次事件的移动速度(px/s)
acc = 速度的变化率(px/s²) = 瞬时加速度
```

- `acc / 阈值 > 1` → 冒泡；**甩得越猛 → 冷却越短 → 冒泡越勤**（下限 0.7s，防刷屏）
- **字号随力度长大**：`13px × 力度^1.25`，封顶 ≈ **鲸鱼娘的身高**（0.72 倍时约 123px）
  —— 所以最猛那一下的「啊啊啊啊啊——」能跟她一样大；气泡的 padding/边框都是 `em`，整颗泡泡一起长大
- `acc / 阈值 ≥ 3` → 换成**尖叫语录**：「啊啊啊啊啊——」（默认 3 条，可自己加）
...
| 指数衰减 | 1.2 | 绝对阻尼 λ |
| 重力 | 1.00 | 沉降量 ≈ `g/k` |
| 甩动灵敏度 | 45k | 鼠标瞬时加速度阈值（px/s²），调小更容易冒泡 |
| 气泡冷却上限 | 4.5 s | 实际冷却 = 上限 ÷ 甩动强度（下限 0.7s） |
| 气泡语录 | 10 条 | 一行一条，改完立即生效 |
| 尖叫语录 | 3 条 | 加速度超阈值 3 倍时专用，**字号能长到和鲸鱼娘一样大** |
