...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		try { if (typeof window !== 'undefined') window.__WHALE_ROD_CURSOR_VER__ = '2026-09-15-v8-swingbubble'; } catch (e) { /* ignore */ }
		...
		function RodCursor() {
			...
			react.useEffect(() => {
				...
				function frame(now) {
					...
					const hx = bx + hdx * Math.cos(radM) - hdy * Math.sin(radM);
					const hy = by + hdx * Math.sin(radM) + hdy * Math.cos(radM);
					if (rod) {
					...
					if (wrap) wrap.style.transform = 'translate3d(' + hx.toFixed(2) + 'px,' + (hy - 8 * S).toFixed(2) + 'px,0)';
